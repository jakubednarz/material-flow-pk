...
def read_all_materials(session: SessionDep):
    ...
    return [MaterialSchema(**material.model_dump()) for material in materials]
...
def update_material_stock(material_id: uuid.UUID, quantity: float, session: SessionDep):
    db_material = session.get(Material, material_id)
    if db_material is None:
        raise HTTPException(status_code=404, detail="Material not found")
...
def read_material_reservations(material_id: uuid.UUID, session: SessionDep):
    ...
    return [reservation.model_dump() for reservation in material.reservations]
...
def read_material_bom_items(material_id: uuid.UUID, session: SessionDep):
    ...
    return [bom_item.model_dump() for bom_item in material.bom_items]
